package net.yingsonic.yra;

public interface RAConstants {
    public static final String MOD_ID = "yra";

    public interface Items {
        String ZIRCONIUM_INGOT = "zirconium_ingot";
        String RAW_ZIRCONIUM = "raw_zirconium";
        String HEALING_WATER_BUCKET = "healing_water_bucket";
    }

    public interface Blocks {
        String ZIRCONIUM_ORE = "zirconium_ore";
        String HEALING_WATER_BLOCK = "healing_water_block";
    }

    public interface Fluids {
        String HEALING_WATER = "healing_water";
        String FLOWING_HEALING_WATER = "flowing_healing_water";
    }

    public interface Effects {
        String HEALING = "healing";
    }

    public interface Potions {
        String HEALING_WATER_POTION = "healing_water_potion";
    }
}
